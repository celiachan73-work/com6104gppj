...
const PathFinderPro = () => {
  ...
  useEffect(() => {
    if (messages.length === 0) {
      setMessages([
        {
          content:
            "你好！我是 PathFinder Pro 升學顧問。我可以幫助你了解在香港升學的各種選擇和機會。你可以詢問關於：\n• 大學課程資訊\n• 入學要求\n• 申請流程\n• 獎學金機會\n請告訴我你想了解什麼？",
          sender: "bot",
    ...
    <div className="flex items-center space-x-2">
      <span
        className={`flex items-center ${
          isRagMode ? "text-blue-500" : "text-gray-500"
        }`}
      >
  ...
  return (
    ...
    >
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2">
                ...
                <div className="relative group">
                  ...
                  <div
                    className={`absolute right-0 top-full mt-2 w-48 p-2 rounded-lg text-xs transform scale-0 group-hover:scale-100 transition-transform origin-top-right z-50 shadow-lg ${
                      isDarkMode
      ...
      {/* Mode Status Bar */}
      <div
        className={`px-4 py-1 ${isDarkMode ? "bg-gray-800/50" : "bg-gray-100"}`}
      >
          ...
          >
            <AnimatePresence>
              {messages.map((message, index) => (
                ...
                >
                  <div
                    className={`flex items-start max-w-[70%] space-x-2 ${
                      message.sender === "user"
                        ? "flex-row-reverse"
                        : "flex-row"
                    }`}
